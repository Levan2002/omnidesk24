...
void HostSession::encodeLoop() {
    ...
    while (running_) {
        ...
        // Check if encoder prefers BGRA (e.g. OmniCodec) to skip I420 conversion
        Frame* encodeFramePtr;
        Frame resizedFrame;
        auto encInfo = encoder_->getInfo();
        if (encInfo.preferredInputFormat == PixelFormat::BGRA ||
            encInfo.preferredInputFormat == PixelFormat::RGBA) {
            // BGRA fast path: encode directly from captured frame (no color conversion)
            encodeFramePtr = &capturedFrame;
        } else {
